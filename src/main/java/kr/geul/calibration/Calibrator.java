...
public abstract class Calibrator {
	...
	class LeastSquaresFunction implements MultivariateFunction {
		...
		public double value(double[] arg0) {
			...
			if (modelType.equals("BS") && arg0[0] <= 0) {
				return -arg0[0] * 10000000.0 + 10000000.0;
			}
			
			if ((modelType.equals("SV") || modelType.equals("SVJ")) 
					&& 2.0 * arg0[0] * arg0[1] <= Math.pow(arg0[3], 2)) {
				return (Math.pow(arg0[3], 2) - arg0[0] * arg0[1]) * 10000000.0 + 10000000.0;
			}
			...
			for (int i = 0; i < allOptions.size(); i++) {
				...
				try {
					modelOption = getModelOption(option, arg0);
						...
						| InconsistentArgumentLengthException | AtTheMoneyException e) {
					e.printStackTrace();
				}
			...
			for (int i = 0; i < arg0.length; i++) {
				params += paramNames[i] + ":" + arg0[i];
					if (i < arg0.length - 1)
						params += ", ";
			}
						
			System.out.println(params + ", ErrorSum: " + totalSquaredError);
